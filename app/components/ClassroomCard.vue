...
<template>
  <div
    ...
    <div class="flex flex-row w-full justify-between items-center gap-2">
      <FontAwesomeIcon class="size-3 text-white" :icon="faUser" />
      <span class="text-sm mr-auto">{{ props.createdBy }}</span>
    </div>
    <div class="flex flex-row w-full justify-between items-center gap-2">
      <FontAwesomeIcon class="size-3 text-white" :icon="faCalendar" />
      <span class="text-sm mr-auto">{{ props.date }}</span>
    </div>
    <div class="flex flex-row w-full justify-end items-center gap-2">
      <div class="size-6">
        <Button class="p-1" @click.stop="deleteClassroom">
          <FontAwesomeIcon class="size-3 text-white" :disabled="isDeleting" :icon="faTrash" />
        </Button>
      </div>
      <div class="size-6">
        <Button class="p-1" @click.stop="refreshClassroom">
          <!-- 加载 -->
          <FontAwesomeIcon class="size-3 text-white" :class="{ 'animate-spin': isRefreshing }"
            :disabled="isRefreshing" :icon="faRefresh" />
        </Button>
      </div>
    </div>
  </div>
