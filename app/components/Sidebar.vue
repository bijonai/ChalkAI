    ...
    <div class="size-full flex flex-col gap-2">
      <div class="size-8">
        <Button @click="collapsed = !collapsed">
          <FontAwesomeIcon class="size-4" :icon="collapsed ? faAngleRight : faAngleLeft" />
        </Button>
      </div>
      <div class="w-full flex flex-col gap-2">
        <div v-for="item in items" :key="item.name" class="w-full h-8 flex flex-row gap-2">
          <Button class="flex flex-row gap-2 justify-start" @click="router.push(item.link)">
            <FontAwesomeIcon class="size-3" :icon="item.icon" />
            <span class="text-sm" v-if="!collapsed">{{ item.name }}</span>
          </Button>
        </div>
      </div>
    </div>
